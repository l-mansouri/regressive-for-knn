...
/* 
 *Variation on  Main regressive-msa-analysis original pipeline script 
 *
 * @authors
    ...
    when:
     ...
     """
       ...
       t_coffee -other_pg aln_compare \
             -al1 ${ref_alignment} \
             -al2 ${test_alignment} \
            -compare_mode sp -io_format p \
            > "${id}.${align_type}.${bucket_size}.${align_method}.${tree_method}.pair_sp"

       t_coffee -other_pg aln_compare \
             -al1 ${ref_alignment} \
             -al2 ${test_alignment} \
            -compare_mode sp -io_format s \
            > "${id}.${align_type}.${bucket_size}.${align_method}.${tree_method}.avg_sp"
       
       ## Total Column Score ##	
       t_coffee -other_pg aln_compare \
             -al1 ${ref_alignment} \
             -al2 ${test_alignment} \
            -compare_mode tc -io_format p \
            > "${id}.${align_type}.${bucket_size}.${align_method}.${tree_method}.pair_tc"

       t_coffee -other_pg aln_compare \
             -al1 ${ref_alignment} \
             -al2 ${test_alignment} \
            -compare_mode tc -io_format s \
            > "${id}.${align_type}.${bucket_size}.${align_method}.${tree_method}.avg_tc"

        ##pairwise identity among the references ## 
        t_coffee -other_pg seq_reformat -in ${ref_alignment} -output sim |grep 'TOP' > "${id}.pairwise_identity"
